...
if __name__ == "__main__":
    ...
    for i in range(0,11): # i  for i_room_x: the same camera
        for j in range(0,file_num[i]): # j for x_room_j: the different room
            room.append(o3d.io.read_point_cloud("data/" + str(i) + "_room_" + str(j) + ".ply"))
            room_num = room_num + 1
    
    room_num = 2 # 0 for room0_0, 1 for room0_1
    flag = 1
    # start to handle the room
    for i in range(1,11): # i  for i_room_x: the same camera
    ...
    o3d.visualization.draw_geometries([room[0], room[1], room[2], room[3], room[4], room[5], room[6], room[7], room[8], room[9], room[10], room[11], room[12], room[13], room[14], room[15], room[16], room[17], room[18], room[19], room[20], room[21], room[22], room[23], room[24], room[25]])
